...
def axis(axis_id, y):
    return GridAxisCandidate(axis_id, "family-1", Point2D(0, y), Point2D(100, y), ("segment-1",), 100.0, 1.0, float(y), 0.0)


def evidence(evidence_id, text, center, *, confidence=0.95):
    x, y = center
    return OcrLabelEvidence(
        id=evidence_id, source_id="drawing-1", page_id="page-1", text=text,
        bbox=BoundingBox2D(Point2D(x - 1, y - 1), Point2D(x + 1, y + 1)),
        confidence=confidence, engine="fixture-ocr", engine_version="1.0", region_kind="grid-bubble",
    )
...
def test_competing_nearby_labels_require_review():
    axes = [axis("gx-1", 0)]
    items = [evidence("ocr-a", "A", (0, 4)), evidence("ocr-b", "B", (0, 4.5))]
    result = associate_grid_labels(axes, items, max_endpoint_distance=10, ambiguity_ratio=1.25)
...
def test_duplicate_normalized_labels_require_review():
    axes = [axis("gx-1", 0), axis("gx-2", 50)]
    items = [evidence("ocr-1", "A", (0, 2)), evidence("ocr-2", " a ", (0, 52))]
...
def test_low_confidence_label_is_not_auto_accepted():
    ...
    result = associate_grid_labels(
        axes, [evidence("ocr-1", "A", (0, 3), confidence=0.5)],
        max_endpoint_distance=10, min_ocr_confidence=0.75,
    )
    assert result[0].status == "review-required"
