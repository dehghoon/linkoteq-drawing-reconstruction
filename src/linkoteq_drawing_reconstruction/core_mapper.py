...
def map_minimal_structural_model(
    *,
    ...
    """Map only currently accepted foundation facts to a Core v0.5 StructuralModel.

    This mapper intentionally emits no members, materials, sections, surfaces,
    supports, loads, or analysis records at the foundation stage. Reconstruction
    evidence remains importer-private.
    """
    if not transform.is_resolved:
        raise UnresolvedTransformError(
            "Core mapping is blocked until T_source_to_model is fully resolved."
        )
    ...
    for node in nodes:
        if node.level_id is not None and node.level_id not in level_ids:
            raise CoreMappingError(
                f"Node {node.id!r} references unknown levelId {node.level_id!r}."
            )
